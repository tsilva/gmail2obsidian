...
- Task format: configurable via `ENTRY_HEADER` (`true`/`false`), `ENTRY_PREFIX` (`checkbox`/`bullet`/`none`), and `ENTRY_LINK` (`true`/`false`); defaults to `- [ ] [subject](gmail-permalink)` under `## Flushed YYYY-MM-DD` headers (header can be disabled)
